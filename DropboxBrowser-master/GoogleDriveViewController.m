...
static NSString *const kKeychainItemName = @"MyLife";
static NSString *const kClientID = @"57810123862-85j75i8i2tqc4lphvf464rackftihj7g.apps.googleusercontent.com";
static NSString *const kClientSecret = @"zeCFDqEBxCUih3uIvrOEvWLm";
...
@synthesize strFilePath, strFolderId;
...
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"DropboxBrowserCell"];
    if (cell == nil) {
    ...
    NSLog(@"icon:%@",file.iconLink);
    NSURL * url = [NSURL URLWithString:file.iconLink];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    GTLDriveFile *file = [self.driveFiles objectAtIndex:indexPath.row];
    NSString *strTitle = file.mimeType;
    
    if([strTitle isEqualToString:@"application/vnd.google-apps.folder"]){
        NSString *strPath = [NSString stringWithFormat:@"%@/%@",self.strFilePath,file.title]; 
        NSLog(@"path:%@",strPath);
        
        self.subdirectoryController = [[GoogleDriveViewController alloc] init];
        self.subdirectoryController.strFolderId = file.identifier;
        self.subdirectoryController.strFilePath = strPath;
//        self.subdirectoryController.bShouldLogout = NO;
//        [self.subdirectoryController listDirectoryAtPath:subpath];
        ...
        SoundRecordViewController *SRViewController = [storyboard instantiateViewControllerWithIdentifier:@"sRecoderView"];
        NSString *strPath = [NSString stringWithFormat:@"%@/%@",self.strFilePath,file.title]; //file.selfLink;
        
        //            [self.navigationController pushViewController:SRViewController animated:YES];
        NSLog(@"path:%@",strPath);
        SRViewController.strFilePath = strPath;
        [self.navigationController presentViewController:SRViewController animated:YES completion:nil];
    }
}
...
- (void)loadDriveFiles {
    ...
    query.q = [NSString stringWithFormat:@"'%@' IN parents", self.strFolderId];
